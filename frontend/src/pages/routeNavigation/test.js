...
import 'i18n-iso-countries/langs/ko.json'; // 한국어 지원
...
const initLocations = [
    { name: '스타벅스', img: '/Img/osaka/img.png' },
...
const MapPage = () => {
    ...
    // const [initCountry, setInitCountry] = useState({lat:'', lng:''})
    ...
    useEffect(() => {
        const fetchCoordinates = async () => {
            try {
                ...
                // 한글 국가명을 영어로 변환
                const englishCountry = countries.getAlpha3Code(country, 'ko');

                // 영어 이름으로 API 요청
                const response = await axios.get(`https://restcountries.com/v3.1/alpha/${englishCountry}`);
                if (response.data) {
                    ...
                    <LoadScript googleMapsApiKey={apiKey} libraries={[librariesPlace]}>
                        {coordinates ? (
                            <GoogleMap
                                ...
                                center={{lat: coordinates.lat, lng: coordinates.lng}} // 오사카 위치
                                zoom={11}
                            ...
                            <div>Loading Coordinates...</div> // coordinates를 로딩 중일 때 표시
                        )}
                    </LoadScript>
                </div>
